...
- Replace the current `ESC *c4660d37e5F` current-state boundary, host-fetched `ESC )s0W`
  parser/route boundary through `0x15d0a`, host-fetched `ESC )s80W` resource-payload
  boundary through `0x16c14` -> `0x16fae` -> `0x1719c`, and host-fetched `ESC )s2193W`
  parser/object boundary through `0x16c14` -> `0x16498` downloaded-pointer `0x1f264`.
  Replace these with a full live parser-state run that populates current records/source
  objects, then replace producer-modeled fixtures with full parser/page-object rendering.
...
- Replace the current `ESC *c4660d37e5F` current-state boundary, host-fetched `ESC )s0W`
  ...
  parser/object boundary through `0x16c14` -> `0x16498` downloaded-pointer `0x1f264`.
  Replace these with a full live parser-state run that populates current records/source
  objects; the verified built-in scan does not provide normal built-in entries for these
  renderer modes.
