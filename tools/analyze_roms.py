...
def compact_bucket_allocator_report(data: bytes) -> str:
    ...
    lines.append("- `tools/render_fixture_harness.py` now has executable `0x1387c` fixtures for short reuse, full-object new-head allocation, segmented tall-glyph bucket allocation/reuse, and printable, mixed printable/control, printable/reset, margin-positioned printable, horizontal/vertical cursor-positioned printable, vertical-layout-positioned printable, simple macro execute, and mixed-control macro execute byte streams that queue through page-record storage before bridging through `0x1edc6`; the plain `!!` path is tied to two ROM parser `0xd04a` events, the mixed `ESC &k1G!\\r!` path is tied to ROM parser handlers `0xedf8`, `0xd04a`, `0xf02c`, and `0xd04a`, the margin `ESC &a1L!` path is tied to `0xeb58` then `0xd04a`, the cursor-position `ESC &a2C!` and `ESC &a1R!` paths are tied to `0xf39e`/`0xf560` then `0xd04a`, the top-margin `ESC &l3E!` path is tied to `0xece2` then `0xd04a`, the macro execute replayed `!\\r` path is tied to parser handlers `0xd04a` and `0xf02c`, the macro execute replayed `ESC &k1G!\\r!` path is tied to `0xedf8`, `0xd04a`, `0xf02c`, and `0xd04a`, and the harness also renders a short object queued through that page-record allocator before bridging it through `0x1edc6`.")
