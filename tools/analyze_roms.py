...
def printable_text_path_report(firmware: bytes) -> str:
    ...
    lines.append("- The live parser path uses the same mapped glyph byte and context fields documented in `ic30_ic13_text_glyph_index_flow.md`; the symbol-set stream fixture now proves `ESC (2U`/`ESC )0E` update active primary/secondary words before `0x14f16` patches the `LINE_PRINTER` map; the next byte-to-pixel model must therefore drive `0xd04a`/`0x1393a`, not feed renderer glyph bytes directly from the host stream.")
    lines.append("- The paired cursor/queue/span behavior after `0x1393a` is detailed in `ic30_ic13_text_cursor_span_flow.md`; `tools/render_fixture_harness.py` now has one-byte and two-byte normal printable stream fixtures for `0x21` -> glyph `0x20` through `0xd824`, the simple `0xd550` default-advance branch, `0x12f2e`, and rendering. It also renders the initialized `LINE_PRINTER` HMI case, where `0x10550(0x00480000)` produces advance `0x00120000` and the second glyph compact coord `0x0202` decodes to `$a001 = 0x12` / pixel x `34`. The plain `!!` stream now has a ROM-parser trace through two `0xd04a` printable events tied to a page-record variant that allocates/reuses the compact object through `0x1387c` and bridges it through `0x1edc6`. The mixed `ESC &k1G!\\r!` fixture proves that line-termination mode is applied before the second printable byte is positioned, queueing it at coord `0x3b00` after CR+LF; the same stream now has a ROM-parser trace through `0xedf8`, `0xd04a`, `0xf02c`, and `0xd04a` tied to a page-record variant that allocates/reuses the compact object through `0x1387c` and bridges it through `0x1edc6`; `ESC &a1L!` now ties margin handler `0xeb58` to page-record queueing and renders the shifted glyph at compact coord `0x0801`; `ESC &a2C!` ties cursor-position handler `0xf39e` to page-record queueing and renders the shifted glyph at compact coord `0x0a02`; the mixed `!\\x1bE` fixture proves reset publication/clear state after queued text and has a page-record allocator/bridge/publication variant for the pre-reset object. The `0x1387c` allocator fixture now queues a short compact object into page-record bucket-array shape and covers the segmented tall-glyph page-record bucket sequence, and the `0x1edc6` bridge fixture proves how that compact bucket and context slot are copied into the render record. The remaining integration gap is to replace fixture-only state with fuller parser-allocated page roots before replacing the current producer-modeled text bucket fixtures.")
    lines.append("")
    return "\n".join(lines)
...
def active_symbol_set_flow_report(data: bytes) -> str:
    ...
    lines.append("- Rebuild the selected primary/secondary character-to-glyph map after symbol-set changes, then apply the `0x14f16` patch rules documented in `ic30_ic13_symbol_set_patch_tables.md`; `tools/render_fixture_harness.py` now drives `ESC (2U` and `ESC )0E` through the symbol-set stream model and applies the resulting `0x0055` patch-table and `0x0005` Roman Extension map rules to the `LINE_PRINTER` base map.")
...
def compact_bucket_allocator_report(data: bytes) -> str:
    ...
    lines.append("- `tools/render_fixture_harness.py` now has executable `0x1387c` fixtures for short reuse, full-object new-head allocation, segmented tall-glyph bucket allocation/reuse, and printable, mixed printable/control, printable/reset, margin-positioned printable, cursor-positioned printable, simple macro execute, and mixed-control macro execute byte streams that queue through page-record storage before bridging through `0x1edc6`; the plain `!!` path is tied to two ROM parser `0xd04a` events, the mixed `ESC &k1G!\\r!` path is tied to ROM parser handlers `0xedf8`, `0xd04a`, `0xf02c`, and `0xd04a`, the margin `ESC &a1L!` path is tied to `0xeb58` then `0xd04a`, the cursor-position `ESC &a2C!` path is tied to `0xf39e` then `0xd04a`, the macro execute replayed `!\\r` path is tied to parser handlers `0xd04a` and `0xf02c`, the macro execute replayed `ESC &k1G!\\r!` path is tied to `0xedf8`, `0xd04a`, `0xf02c`, and `0xd04a`, and the harness also renders a short object queued through that page-record allocator before bridging it through `0x1edc6`.")
    lines.append("")
    return "\n".join(lines)
...
def render_dispatch_table_report(data: bytes) -> str:
    ...
    lines.append("It also has modeled raster command/data stream fixtures for `ESC *t300R` / `ESC *r1A` / delayed `ESC *b4W`, `ESC *t150R` / `ESC *r0A` / delayed `ESC *b2W`, `ESC *t100R` / `ESC *r0A` / delayed `ESC *b2W`, and `ESC *t75R` / `ESC *r0A` / delayed `ESC *b2W` bytes that record handler `0x0105d0`, queue mode-0/1/2/3 objects, bridge the `ESC *b4W` page-record object through `0x1edc6`, and render the literal, two-row, three-row, and four-row expansions; the primary 300-dpi stream now has a cross-boundary check tying the ROM parser handlers and `0x12218` restore to the modeled payload offset, queued object, bridge, rendered row, and row counter; a separate `ESC *t300R` / `ESC *r0A` stream with two consecutive uppercase `ESC *b2W` payloads verifies row_y `0 -> 1 -> 2` and page-record chain objects at coords `0x1000` then `0x0000`.")
    lines.append("Same-group lowercase-final chaining fixtures cover `ESC *t300r150R` and chained `ESC *b2w2W`; the lowercase `w` records the delayed transfer and keeps parser mode in the `*b` family, then the uppercase `W` triggers `0x12218` restore/dispatch and consumes the single raster payload. A bare `ESC *rB` stream proves handler `0x107fa` clears only raster active state and allows a following `ESC *t150R` mode change.")
...
def font_control_flow_report(data: bytes) -> str:
    ...
    lines.append("- Nonzero `W` counts schedule `0x16c14`; the harness now ties the full `ESC )s2193W` parser trace to restored handler `0x16c14`, payload offset/length, the `0x16498` downloaded character-object allocation, `0x16874`/`0x16942` payload copy, and the rendered `0x1f264` segmented-wide row. `0x15d0a` reaches the same character-object path only after the descriptor branch resolves an existing object whose flag bit 30 is set.")
...
def rectangle_graphics_flow_report(data: bytes) -> str:
    ...
    lines.append("- `tools/render_fixture_harness.py` now pins dot/decipoint size stores, `ESC *c#G` absolute/clear behavior, `ESC *c#P` selector mapping, a chained `ESC *c12a5b0P` byte stream queueing the selector-7 rule object plus a ROM `0x11774` dispatch trace for the same stream, portrait rule-list object queueing/bridge normalization, solid black selector-7 rendering through `0x1f446`/`0x1f596`, solid and patterned rule band-crossing continuation, a two-band page-row assembly for a crossing HP-pattern rule, gray selectors `0..6` and HP pattern selectors `8..13` through `0x1f446`/`0x1f4e0`, sub-byte HP pattern masks/pixels, negative-left clipping, and the `0x10d22` no-room retry path through `0xff1e`, `0x10084`, `0x13386`, `0x1edc6`, and rule rendering. Remaining work is parser-produced full-page comparisons for these rule paths.")
    lines.append("")
    return "\n".join(lines)
