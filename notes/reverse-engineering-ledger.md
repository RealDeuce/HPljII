...
| Direct control codes | parser mode 0 maps CR/LF/FF/HT/BS to handlers `0xf02c`, `0xf08c`, `0xf0f0`, `0xf1cc`, `0xf2a8`; `ESC &k#G` stores line-termination mode bits in `0x78318f`; CR/LF/FF/HT/BS cursor/page-visible side effects are documented in `generated/analysis/ic30_ic13_direct_control_code_flow.md`; `tools/render_fixture_harness.py` now pins synthetic packed-state fixtures for those direct control-code effects; `ESC E` maps to reset handler `0xcc52` | Anchored for direct control-code side effects; parser-driven fixtures and reset internals incomplete |
...
- Replace the synthetic CR/LF/FF/HT/BS cursor/page fixtures with parser-driven byte-stream fixtures and trace `ESC E` internals into reset-visible renderer state.
...
- Use the documented `0x1393a` / `0x12f2e` text-object glyph-index path to feed realistic selected-context longwords and glyph indices to those fixtures.
