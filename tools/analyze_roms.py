...
def render_dispatch_table_report(data: bytes) -> str:
    ...
    lines.append("")
    lines.append("`tools/render_fixture_harness.py` now has parser-derived raster state fixtures for `0x10808` (`ESC *t#R`) and `0x1075a` (`ESC *r#A`), proving 300/150/100/75 dpi select encoded modes 0/1/2/3 before queueing a mode-0 transfer object; it also has raster row fixtures that queue byte-aligned mode-0 object `00 00 00 00 80 00 00 04 00 01 f0 0f aa 55`, non-byte-aligned mode-0 object `00 00 00 00 80 00 00 02 04 01 c3 3c`, mode-1 object `00 00 00 00 80 01 00 02 00 01 f0 0f`, mode-2 object `00 00 00 00 80 02 00 02 00 01 f0 0f`, and mode-3 object `00 00 00 00 80 03 00 02 00 01 f0 0f` through the `0x13070` / `0x13250` / `0x138de` shape, bridge the byte-aligned mode-0 object through `0x1edc6`, and render the rows through `0x1f88e` / `0x1f8da`, `0x1f8e6`, `0x1f920`, and `0x1f9c6`.")
    lines.append("")
    return "\n".join(lines)
