...
def rectangle_graphics_flow_report(data: bytes) -> str:
    ...
    lines.append("- `tools/render_fixture_harness.py` now pins dot/decipoint size stores, `ESC *c#G` absolute/clear behavior, `ESC *c#P` selector mapping, portrait rule-list object queueing/bridge normalization, solid black selector-7 rendering through `0x1f446`/`0x1f596`, gray selectors `0..6` and HP pattern selectors `8..13` through `0x1f446`/`0x1f4e0`, and negative-left clipping. Remaining work is sub-byte, band-crossing, and full-page comparisons for these rule paths.")
    lines.append("")
    return "\n".join(lines)
