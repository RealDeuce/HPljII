...
def text_cursor_span_report(firmware: bytes) -> str:
    ...
    lines.append("- The flagged `0xd824` positioning path, including the negative-left overflow branch, has executable queue/render fixtures in `tools/render_fixture_harness.py`, and printable stream fixtures now carry host byte `0x21` through `0x1393a`, `0xd824`, the simple `0xd550` default-advance branch for a second byte, `0x12f2e`, and rendering. The initialized `LINE_PRINTER` HMI case is queued through the same path and exposes compact coord `0x0202`, whose sub-byte phase is not rendered yet. Synthetic `0xd3b2` fixtures cover both unflagged positioning branches but do not yet implement inline/downloaded `0x12f2e` payloads. Remaining work is to use real parser-produced source objects, implement the inline/downloaded payload path, implement sub-byte compact rendering, and name the coordinate axes by comparing orientation, CR/LF/FF, and raster placement behavior.")
    lines.append("")
    return "\n".join(lines)
