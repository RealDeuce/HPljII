...
source `0xa3ca`, dirty-record commit/readback helpers `0x96c4`/`0x97e4`, and
serial bit helpers `0x9860`/`0x98ae`/`0x994e`/`0x9a4a`, including `$a400`
phase pairs `1 -> 3`, `5 -> 7`, and `1 -> 0`
...
low-nibble, and byte-boundary endpoints; remaining descriptor work is
field-changing selected-font state named in
`notes/font-context-metrics.md#selected-font-residual-routing-checkpoint` and
external/manual naming for consumed-but-not-staged fields. Those pinned metric
endpoints include rounded input `0x0013` copying `+0x2c =
0x0014`, high-byte rounded inputs `0x1500`/`0x1508`/`0x15ff` all copying `+0x2c =
0x0060`, max positive and max negative copied offset words `0x007f`/`0xffff`,
lower-bound equality, exact page-extent equality, byte-boundary outputs
`0x00fc`/`0x0100`/`0x0104`, and mixed legal values `0x0008/0x0030/0x002a/0x02` copying
`+0x18/+0x1a/+0x2c = 0x0027/0x0002/0x002c`, rounded `0x00ff` capping to `+0x2c =
0x00c0`, offset byte `0x80` sign-extending to `+0x1a = 0xff80`, and late first-code
`0x002f` deriving `+0x18 = 0`. range-cap output `0x0100`. ROM-internal validation/error
page behavior is no longer an open class: `0x16fae..0x17016` predicate failures for
entries `2`, `4`, `5`, `6`, and `7` plus the short-budget entry-`5` case are covered by
fixtures `ESC )s80W additional validation predicate failures skip allocation` and `ESC
)s#W validation failures preserve following printable output`, while `0x16498`
...
`0x1f88e`, `0x1effe`/`0x1f0d2`, and `0x1f596` before recording composed rows,
...
records the same ROM-derived rows,
...
- Trace the remaining physical panel, power-on, and NVRAM failure edges around
  ...
  through `0x1ba92`, with shared slot deletion/compaction pinned at
  `0x1bd2e`, releases affected `0x782640` current-record payloads through
  ...
  `0x780e08` from the aggregate result. Fixture
  `0x1a2e4 font scan ignores scheduler return` proves the font-resource scan
  caller snapshots `0x78278e` to `0x782780`, ignores scheduler `D7`, passes
  `0x78219b/0x78219c` plus local `A6-0x02` to `0x1b50e`, and calls `0x6364`
  only from the resolver `D7 == 0` path. The remaining scheduler-specific edge
