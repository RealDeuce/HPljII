...
| Direct control and reset | parser mode 0 maps CR/LF/FF/HT/BS to handlers `0xf02c`, `0xf08c`, `0xf0f0`, `0xf1cc`, `0xf2a8`; `ESC &k#G` stores line-termination mode bits in `0x78318f`; CR/LF/FF/HT/BS cursor/page-visible side effects are documented in `generated/analysis/ic30_ic13_direct_control_code_flow.md`; `tools/render_fixture_harness.py` now pins synthetic packed-state fixtures for those effects plus narrow direct-control byte-stream fixtures for `ESC &k1G`+CR, `ESC &k2G`+LF, and `ESC &k0G`+HT/BS; `ESC E` maps to reset handler `0xcc52`, reset flow is documented in `generated/analysis/ic30_ic13_esc_e_reset_flow.md`, and synthetic `ESC E` byte-stream fixtures cover valid-page-root publication and missing-root clearing | Anchored for direct control-code side effects, narrow byte streams, reset sequencing, and synthetic reset fixtures; full firmware parser/reset fixtures incomplete |
...
- Broaden the documented printable `0x1393a` / `0xd824` / `0xd550` / `0x12f2e` text-object glyph-index fixtures into sub-byte rendering, realistic selected-context longwords, real HMI/font metrics, glyph indices, and parser-produced page objects.
