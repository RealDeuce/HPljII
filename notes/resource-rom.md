...
6. Model the font-printout loop's emitted page objects from the ROM
   ...
   documented below, and the first eight bytes of sample run 1 are now
   consumed through the `0x1c5e8..0x1ed84` page-object/render boundary by
   fixture `font sample run 1 prefix crosses page-record render entry`.
   The next implementation target is extending that fixture from
   `ABCDEfgh` to full row and full printout placement. Compare those
