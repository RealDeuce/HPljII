...
def direct_control_code_flow_report(data: bytes) -> str:
    ...
    lines.append("- Axis names remain provisional, but `tools/render_fixture_harness.py` now has synthetic state fixtures for the line-termination map plus CR/LF/FF/HT/BS cursor/page effects, `ESC &f#S` cursor stack push/pop and clamp behavior, `ESC &a#C/#H/#R/#V` cursor-position conversion/relative/clamp behavior, `ESC &a#L/#M` margin conversion/reject/cursor-move behavior, narrow byte-stream fixtures for `ESC &k1G`+CR, `ESC &k2G`+LF, `ESC &k2G`+FF, `ESC &k3G`+CR/LF/FF, `ESC &k0G`+HT/BS, and `ESC &f0S`/`ESC &f1S` through selector handler `0xf75e`, a mixed `ESC &k1G!\\r!` fixture that applies CR+LF before queueing the second printable glyph, and a mixed `!\\x1bE` fixture that applies reset publication/clear state after queued text and has a page-record allocator/bridge variant for the pre-reset glyph. The remaining step is expanding this into the full firmware parser path with real page-object allocation.")
    lines.append("")
    return "\n".join(lines)
