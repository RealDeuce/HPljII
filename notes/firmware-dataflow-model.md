...
- Compact text and downloaded-glyph objects:
  ...
  Bridge path is `0x1ed84 -> 0x1edc6 -> 0x1ef6a`; bucket dispatch is
  `0x1efc2 -> 0x1effe`, then compact helper `0x1f034`, `0x1f0d2`,
  `0x1f1f0`, or `0x1f264` depending on selector bits. Output rows come from
  source glyph/resource bytes resolved through `0x1f354` and row-copy helpers.
- Portrait text-span segment-list objects:
  ...
  Bridge path is `0x1ed84 -> 0x1edc6 -> 0x1ef6a`; bucket dispatch is
  `0x1efc2 -> 0x1f812 -> 0x1f862`. Output rows are span masks and full words
  derived from the queued segment entries, not from parser scratch.
- Encoded raster span objects:
  ...
  Bridge path is `0x1ed84 -> 0x1edc6 -> 0x1ef6a`; bucket dispatch is
  `0x1efc2 -> 0x1f88e`. Object byte `+0x05 & 3` selects literal or expansion
  mode helpers, and the queued payload bytes produce the rendered raster rows.
- Rectangle/rule objects:
  ...
  Bridge path is `0x1ed84 -> 0x1edc6 -> 0x1ef6a`; render dispatch is
  `0x1f446`, which sends bridged selector `7` to solid helper `0x1f596` and
  other documented selectors to patterned helper `0x1f4e0`.
- Fixed-list and landscape span objects:
  ...
  `+0x04..+0x0d`. Bridge path is `0x1ed84 -> 0x1edc6 -> 0x1ef6a`; render
  dispatch is `0x1f756`, gated on five-band boundaries, then row writing
...
Pixel composition contract:

- The shared render entry composes by call order, not by an implicit graphics
  mode: bucket-chain objects render first, rule-list objects second, and
  fixed-list objects last.
- The documented pixel helpers write generated bytes, words, or longwords
  directly to the active band buffer rooted at `0x783a28`, or to fallback
  buffer storage rooted at `0x7810b4 + byte_pair_offset` when a helper carries
  rows past the current band.
- The current documented helper set does not read the previous destination
  word and OR/XOR/AND it with earlier pixels. Patterned-rule helper `0x1f4e0`
  masks its generated pattern word before storing; that mask is part of the
  generated source word, not a destination read-modify-write blend.
- Continuation fields copied or initialized by `0x1edc6`, such as rule word
  `+0x0c` and fixed-list word `+0x0a`, are render bookkeeping consumed across
  later band calls. They are not parser state and should not be recomputed from
  host bytes during rendering.
