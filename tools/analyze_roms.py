...
def active_symbol_set_flow_report(data: bytes) -> str:
    ...
    lines.append("The executable harness now models both `0x1ac0a` branches and both `0x1af36` fallback branches: current-candidate mode copies `0x7828a4` into the relevant slots, while synthesized mode records one word per `0x78289f` orientation and `0x78289e` primary/secondary selector. It also pins the `0x1ad66` control flow: try `0x1adaa(1)` for `0x200000..0x3ffffe`, try `0x1adaa(2)` for `0x400000..0x5ffffe`, and fall back through `0x1ae7e` to either a `0x1b060` match or the bit-30-selected `0x15890`/`0x158be` base-candidate reader. The remaining live-state gap is inside `0x1b250`, `0x1ab84`, the `0x1bbfe`/`0x1b060` helper returns, and the real candidate records feeding those searches, not the table writes, `@` parser exposure, or `0x1ad66` list/range/fallback control flow.")
    lines.append("")
