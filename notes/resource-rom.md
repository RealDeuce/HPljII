...
Filtering helpers around `0x1519a`, `0x153c6`, `0x147f4`, and `0x148f8` prune the active list by attributes such as pitch, style, symbol set, and current orientation-specific state. `0x1519a` is now pinned as the height filter: it reads primary/secondary requested height from `0x782ef2` / `0x782f02`, first keeps active candidates within +/-`0x19`, and if none match uses `0x1533e` to choose the nearest lower and/or upper decoded height. Built-in heights are decoded through `0x13bca` from record `+0x28/+0x2a`; the verified class-zero window splits into eight `1200`-unit candidates and four `850`-unit candidates. This directly affects text rendering because it determines the built-in font record used for glyph metrics and bitmaps.
