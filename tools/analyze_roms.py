...
def text_cursor_span_report(firmware: bytes) -> str:
    ...
    lines.append("- The flagged `0xd824` positioning path, including the negative-left overflow branch, has executable queue/render fixtures in `tools/render_fixture_harness.py`, and printable stream fixtures now carry host byte `0x21` through `0x1393a`, `0xd824`, the simple `0xd550` default-advance branch for a second byte, `0x12f2e`, and rendering. The initialized `LINE_PRINTER` HMI case renders through the same path and proves compact coord `0x0202` as `$a001 = 0x12` / pixel x `34`; the mixed `ESC &k1G!\\r!` case proves CR+LF repositioning before the second printable byte and exposes full-byte shifted blank-row clearing; the mixed `!\\x1bE` case proves reset publication/clear state after queued text. A `0x1387c` fixture queues one compact bucket into page-record shape, and a `0x1edc6` fixture bridges that compact bucket into render-record shape and pins rule/fixed-list normalization. Synthetic `0xd3b2` fixtures cover both unflagged positioning branches but do not yet implement inline/downloaded `0x12f2e` payloads. Remaining work is to use real parser-produced source/page objects, implement the inline/downloaded payload path, and name the coordinate axes by comparing orientation, CR/LF/FF, and raster placement behavior.")
    lines.append("")
    return "\n".join(lines)
...
def compact_bucket_allocator_report(data: bytes) -> str:
    ...
    lines.append("| `0x12f70..0x12fbe` | selects flagged or unflagged source metrics and sets selector bit `0x1000` for wide glyphs | selector byte `+4` bit `0x10` chooses compact render mode 1 when width exceeds the threshold |")
    ...
    lines.append("- `tools/render_fixture_harness.py` now has executable `0x1387c` fixtures for reuse and full-object new-head allocation, then renders a short object queued through that page-record allocator before bridging it through `0x1edc6`.")
    lines.append("")
    return "\n".join(lines)
...
def direct_control_code_flow_report(data: bytes) -> str:
    ...
    lines.append("- Axis names remain provisional, but `tools/render_fixture_harness.py` now has synthetic state fixtures for the line-termination map plus CR/LF/FF/HT/BS cursor/page effects, narrow byte-stream fixtures for `ESC &k1G`+CR, `ESC &k2G`+LF, and `ESC &k0G`+HT/BS, a mixed `ESC &k1G!\\r!` fixture that applies CR+LF before queueing the second printable glyph, and a mixed `!\\x1bE` fixture that applies reset publication/clear state after queued text. The remaining step is expanding this into the full firmware parser path with real page-object allocation.")
    lines.append("")
    return "\n".join(lines)
