...
The mixed text/control stream `ESC &k1G!\r!` now has an explicit parser-to-page-record check: the ROM dispatch trace reaches handlers `0xedf8`, `0xd04a`, `0xf02c`, and `0xd04a`, and the same byte stream allocates one page-record root, reuses compact bucket `0`, bridges through `0x1edc6`, and renders the expected post-CR rows. `ESC &a1L!` and `ESC &a2C!` now have the same boundary coverage for left-margin handler `0xeb58` and cursor-position handler `0xf39e` followed by printable `0xd04a`, queueing glyphs at compact coords `0x0801` and `0x0a02`.
