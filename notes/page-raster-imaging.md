...
`generated/analysis/ic30_ic13_page_root_allocation.md` now details the `0x10084` / `0x10110` allocation boundary. `0x10084` ensures the current page object root exists. On first allocation, it:
...
The executable harness `tools/render_fixture_harness.py` combines the host-byte fetch, tokenizer/delayed-payload, page-geometry, macro/data-chain, direct-control, reset, text, rule, raster, bridge, row-copy, built-in glyph, symbol-set, and downloaded-font fixture families into one ROM-backed self-test. It emits `generated/analysis/ic30_ic13_renderer_fixture_harness.md` and currently verifies 247 checks. The raster coverage now includes a ROM-table `0x11774` dispatch trace for the primary `ESC *t300R` / `ESC *r1A` / `ESC *b4W` stream, modeled delayed `0x121cc` / `0x12218` transfer records, command/data-stream transfer events routed through the modeled `0x105d0` gate including capped-byte and beyond-extent drain-without-queue/no-row-advance cases, page-record queue/bridge/render checks for modes 0..3, and raster object rendering through `0x13070` / `0x13250` / `0x138de` / `0x1edc6` / `0x1f88e`. Rectangle coverage now also has a ROM-table `0x11774` dispatch trace for `ESC *c12a5b0P`, proving the parser selects `0x10e68`, `0x10e22`, and `0x10898` before queueing and rendering the selector-7 rule object. Page-root allocation coverage is now backed by `generated/analysis/ic30_ic13_page_root_allocation.md` and pins the `0x10110` selected-context slot bootstrap after `0x10084` first-root allocation. Direct publication-stream coverage now traces `!\x1bE`, `ESC &k2G!\f`, `!\x1b&l1A`, and `!\x1b&l1O` through the ROM parser path, proving printable fallback to `0xd04a`, reset dispatch to `0xcc52`, line-termination dispatch to `0xedf8`, FF dispatch to `0xf0f0`, page-size dispatch to `0xfc74`, and orientation dispatch to `0x10220` before the modeled page-record publication fixtures run. Macro coverage now has the same ROM-table proof for `ESC &f-123y0x1X`, walking modes `0 -> 1 -> 5 -> 17 -> 17 -> 17 -> 0` to handlers `0xe112`, `0xdd08`, and `0xdd08` before applying the modeled macro state effects; a second macro-definition trace proves alternate table `0x116f6` leaves payload bytes unclaimed while still routing `ESC &f1X` to `0xdd08`, and a stored `ESC &k1G!\r!` macro payload now drains through `0xa904` into the same page-record stream and rendered rows as the direct mixed control-byte model.

This is still not enough for pixel-perfect reproduction by itself. The next unresolved step is to replace fixture-only source/bucket states with fuller parser-produced page objects, replace the modeled font command/data wrappers with a full live parser-state run that populates current records and source/page objects, then replace the producer-modeled text/raster bucket objects with page objects captured or reproduced from the full parser/imaging path. The reset, FF, page-size, and orientation publication fixtures now start without a current page root and mark the first printable queue step as the modeled page-record root allocation point, but that is still not a full live parser allocation. Raster coverage now has a ROM-table `0x11774` dispatch trace for the primary `ESC *t#R` / `ESC *r#A` / `ESC *b#W` stream, but still needs a full CPU/parser-state fixture that executes through `0x121cc` / `0x105d0` and parser-produced page-object allocation. The constructed `0x1f0d2` and `0x1f1f0` inline cases now also have type-2 `0x1719c` payload-backed fixed-record coverage; the `0x1f264` segmented-wide case now has selected-memory isolation plus modeled `ESC )s0W` descriptor and `ESC )s2193W` payload command wrappers paired with a ROM dispatch-table trace through `0x11774` modes `0/1/4/13`, delayed record restoration through `0x121cc` / `0x12218`, descriptor selector routing through `0x15d0a`, payload copying into `0x16498`, and a downloaded-pointer allocation render fixture. The full built-in scan proves the verified ROM resources do not contain a normal wide or non-mode-1 bitmap-entry case.
...
- Replace the modeled font command/data wrappers, especially the `ESC )s0W` -> `0x15d0a` descriptor route and `ESC )s2193W` -> `0x16c14` -> `0x16498` downloaded-pointer `0x1f264` segmented-wide case, with a full live parser-state run that populates current records/source objects; then replace producer-modeled text bucket fixtures with full parser-produced page-object payloads.
