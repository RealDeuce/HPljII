...
- Replace the modeled font command/data wrapper, especially the `ESC )s2193W` -> `0x16c14` -> `0x16498` downloaded-pointer `0x1f264` segmented-wide case, with a full live parser-state run that populates current records/source objects; then replace producer-modeled text bucket fixtures with full parser-produced page-object payloads.
