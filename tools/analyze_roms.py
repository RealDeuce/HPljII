...
def rectangle_graphics_flow_report(data: bytes) -> str:
    ...
    lines.append("- `tools/render_fixture_harness.py` now pins dot/decipoint size stores, `ESC *c#G` absolute/clear behavior, `ESC *c#P` selector mapping, a chained `ESC *c12a5b0P` byte stream queueing the selector-7 rule object, portrait rule-list object queueing/bridge normalization, solid black selector-7 rendering through `0x1f446`/`0x1f596`, solid and patterned rule band-crossing continuation, a two-band page-row assembly for a crossing HP-pattern rule, gray selectors `0..6` and HP pattern selectors `8..13` through `0x1f446`/`0x1f4e0`, sub-byte HP pattern masks/pixels, and negative-left clipping. Remaining work is parser-produced full-page comparisons for these rule paths.")
    lines.append("")
    return "\n".join(lines)
