...
- Replace the remaining selected/synthetic inline fixed-record memory, especially the `0x1f264` segmented-wide case, with records populated by the real font-download parser and a decoded larger payload-allocation path; then replace producer-modeled text bucket fixtures with full parser-produced page-object payloads.
