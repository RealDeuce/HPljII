...
The executable harness `tools/render_fixture_harness.py` combines the host-byte fetch, tokenizer/delayed-payload, page-geometry, macro/data-chain, direct-control, reset, text, rule, raster, bridge, row-copy, built-in glyph, symbol-set, and downloaded-font fixture families into one ROM-backed self-test. It emits `generated/analysis/ic30_ic13_renderer_fixture_harness.md` and currently verifies 234 checks. The raster coverage now includes a ROM-table `0x11774` dispatch trace for the primary `ESC *t300R` / `ESC *r1A` / `ESC *b4W` stream, modeled delayed `0x121cc` / `0x12218` transfer records, page-record queue/bridge/render checks for modes 0..3, and raster object rendering through `0x13070` / `0x13250` / `0x138de` / `0x1edc6` / `0x1f88e`.
