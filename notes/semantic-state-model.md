...
`0x780e39.3` is set; it still does not prove which upstream NVRAM validation
path sets that flag or how failed retained storage falls back into `0x780eda`.
...
- Unknown/provenance:
  ...
  - `0x6f32(0x4c)`, `0x780e39.3`, `0x780e39.4`, and `0x780e31.6/7`
    identify service/error branch conditions, but their upstream writers are
    not composed here.
  - manual notes tie `68 SERVICE` to NVRAM failure, but this checkpoint only
    proves the display handler boundary `0x85c0`; it does not prove fallback
    writes into `0x780eda`.
...
`0x85c0`/`68 SERVICE` display boundary, status-shadow fields, and register
...
board-level device is not identified. Low for NVRAM-failure provenance: manual
notes identify the user-visible meaning of `68 SERVICE`, but the ROM path that
sets `0x780e39.3` or reloads failed retained defaults remains unresolved.
...
- `upstream NVRAM validation -> 0x780e39.3 -> 0x85c0`: this checkpoint proves
  the service-display boundary, but not the retained-storage validation writer
  that sets the bit.
